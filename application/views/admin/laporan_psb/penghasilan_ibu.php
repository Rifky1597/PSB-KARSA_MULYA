...
jQuery(function(){
 new Highcharts.Chart({
  chart: {
   renderTo: 'chart_penghasilan_ibu',
   type: 'pie',
  },
  title: {
  ...
  subtitle: {
   text: 'Jumlah Data Pendaftar Berdasarkan Penghasilan Ibu',
   x: -20
  },
  xAxis: {
   categories: [
   <?php foreach($penghasilan_ibu as $k => $value) {
  ...
  series: [{
    ...
    data: [
    <?php
    foreach($penghasilan_ibu as $k => $value) {
      $key = str_replace("<","Di bawah ",$k);
